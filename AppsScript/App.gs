...
function prepMessagesFromEmail() {
  let preppedMessages = [];
  const allMessages = GmailApp.getMessagesForThreads(
...
function getTransactionsFromAllMessages(preppedMessages) {
  ...
  preppedMessages.forEach((thisMessage) => {
    ...
    allTransactionValues.newCompleted.push(
      ...messageTransactionValues.newCompleted
    );
  });
  ...
  messageSections.forEach((thisSection) => {
    try {
      ...
      if (transType != null) {
        ...
        if (
          [
            TRANSACTION_NAMES.EXPENSE,
            TRANSACTION_NAMES.PENDING_EXPENSE,
          ].includes(transType)
        ) {
        ...
        if (
          [
            TRANSACTION_NAMES.PENDING_EXPENSE,
            TRANSACTION_NAMES.PENDING_DEPOSIT,
          ].includes(transType)
        ) {
...
function getTransactionType(section, bank) {
  const matchingTransType = Object.entries(bank.TRANS_TYPE).find(
    ([typeKey, regex]) => regex.test(section)
  );
...
function reviewPendingTransactionsFromSheet(newCompletedTransactions) {
  if (newCompletedTransactions.length > 0) {
    const allRowsFromTransactionSheet =
      GLOBAL_CONST.TRANSACTIONS_SHEET.getDataRange().getValues();
    let currentPendingTransactions = getCurrentPendingTransactionsFromSheet(
      allRowsFromTransactionSheet
    );
    if (currentPendingTransactions.length > 0) {
      var anyPendingTransactionWasResolved =
        resolveAnyCompletedPendingTransactions(
...
function resolveAnyCompletedPendingTransactions(
  newCompletedTransactions,
  currentPendingTransactions
) {
  let anyPendingTransactionWasResolved = false;
  newCompletedTransactions.forEach((thisNewCompletedTransaction) => {
    thisNewCompletedTransaction[3] = thisNewCompletedTransaction[3].replace(
      /,/g,
      ""
    );
    for (let i = 0; i < currentPendingTransactions.length; i++) {
      let thisCurrentPendingTransaction = currentPendingTransactions[i];
      let currentPendingTransactionForComp = [
        ...thisCurrentPendingTransaction[1],
      ];
      currentPendingTransactionForComp[1] =
        currentPendingTransactionForComp[1].replace("Pending ", "");
      if (
        JSON.stringify(currentPendingTransactionForComp) ===
        JSON.stringify(thisNewCompletedTransaction)
      ) {
        ...
        GLOBAL_CONST.TRANSACTIONS_SHEET.deleteRow(
          thisCurrentPendingTransaction[0]
        );
